...
async function create_new_auth(auth) {
  try {
    const { id_user, verification_code, verification_expiration, refresh_token } =
      auth;
      
    const new_auth = await pool.query(
      `
        INSERT INTO AUTH
        (id_user, verification_code, verification_expiration, refresh_token) 
        VALUES 
        ($1, $2, $3, $4); 
        `,
      [id_user, verification_code, verification_expiration, refresh_token]
